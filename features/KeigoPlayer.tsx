...
const KeigoPlayer: React.FC<KeigoPlayerProps> = ({ script, onEnd }) => {
  ...
  useEffect(() => {
    if (containerRef.current) {
      ...
      const listContainer = scrollContainer.querySelector('.script-list') as HTMLElement;
      if (listContainer && listContainer.children[activeIndex]) {
        ...
        const targetTop = activeEl.offsetTop - (scrollContainer.clientHeight * 0.33);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-white relative">
      ...
      <div className="absolute top-0 left-0 right-0 h-16 bg-white/95 backdrop-blur-xl border-b border-gray-100 z-[9999] flex items-center justify-between px-4">
        <button onClick={onEnd} className="text-gray-400 p-2 active:scale-90 transition-transform">
          <i className="fas fa-chevron-down"></i>
        </button>
        <div className="flex flex-col items-center">
          <span className="font-bold text-[#ff4500] text-[10px] tracking-[0.2em] uppercase leading-none mb-1">Learning</span>
          <span className="font-black text-gray-900 text-sm">KEIGO RADIO</span>
        </div>
        <button onClick={togglePlay} className="text-gray-800 p-2 active:scale-90 transition-transform">
      ...
      >
        ...
        <div className="script-list px-8 pt-[35vh] pb-[45vh] space-y-12">
          {script.map((line, i) => (
            <div 
              key={i} 
              className={`transition-all duration-700 cursor-pointer origin-left py-2 ${i === activeIndex ? 'keigo-active' : 'keigo-inactive'}`}
              onClick={() => {
                setActiveIndex(i);
                if (!isPlaying) setIsPlaying(true);
              }}
            >
              <h2 className="text-3xl font-black mb-3 leading-tight break-keep transition-all duration-500">
                {line.japanese}
              </h2>
              {line.korean && (
                <p className={`text-lg font-medium transition-all duration-700 ${i === activeIndex ? 'opacity-50 text-gray-600 translate-y-0' : 'opacity-0 -translate-y-2'}`}>
                  {line.korean}
                </p>
              )}
            </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-8 pt-6 pb-10 bg-white/95 backdrop-blur-xl border-t border-gray-100 flex flex-col items-center z-[9999]">
        ...
        <div className="flex items-center justify-between w-full max-w-[280px]">
          <button 
            ...
            onClick={() => setActiveIndex(prev => (prev - 1 + script.length) % script.length)}
          >
