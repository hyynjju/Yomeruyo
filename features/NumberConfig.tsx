...
const NumberConfigView: React.FC<NumberConfigViewProps> = ({ onBack, onStart }) => {
  ...
  return (
    <div className="flex flex-col h-full space-y-8 pb-32">
      ...
      <div className="space-y-6">
        <div>
          <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">세는 단위</label>
          <select 
            value={counter} 
        ...
        {!isDateMode && (
          <>
            <div>
              <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">숫자 범위</label>
              <div className="flex items-center space-x-4">
                <input 
                  type="number" 
                  value={min} 
            ...
            <div>
              <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">단위</label>
              <div className="flex flex-wrap gap-2">
                {[1, 10, 100, 1000, 10000].map(u => (
                  <button
                    key={u}
                    onClick={() => setUnit(u as any)}
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${unit === u ? 'bg-[#ff4500] text-white' : 'bg-gray-100 text-gray-500 border border-transparent'}`}
                  >
                    {u}단위
                  </button>
                ))}
        ...
        <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100">
          ...
          <button 
            ...
            className={`w-14 h-8 rounded-full transition-all relative ${showKorean ? 'bg-[#ff4500]' : 'bg-gray-300'}`}
          >
            <div className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-all ${showKorean ? 'left-7' : 'left-1'}`}></div>
          </button>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-6 bg-white z-[9999]">
        <button 
          onClick={() => onStart({ range: [min, max], unit, counter, showKorean })}
