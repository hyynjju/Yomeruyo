...
const PlaceConfigView: React.FC<PlaceConfigViewProps> = ({ onBack, onStart }) => {
  const [category, setCategory] = useState<PlaceConfig['category']>('PREFECTURE');
      ...
      <div className="space-y-4">
        {categories.map((cat) => (
          <button
            ...
            className={`w-full p-4 rounded-xl text-left font-bold transition-all border ${category === cat.id ? 'bg-[#ff4500] border-transparent text-white' : 'bg-gray-50 border-gray-200 text-gray-500'}`}
          >
            {cat.label}
          </button>
        ))}
